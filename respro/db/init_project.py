...
def _load_genbank_records(
    ...
    for record in records:
        ...
        for gene in record.genes:
            created_gene = _get_or_create_gene(conn, reference_id, gene)
            if created_gene:
                inserted_genes += 1
            else:
                reused_genes += 1
...
def _get_or_create_gene(conn: sqlite3.Connection, reference_id: int, gene) -> bool:
    ...
    if existing is None:
        conn.execute(
            'INSERT INTO gene '
            '(reference_id, name, protein, start, end, strand, codon_start, nt_sequence, aa_sequence) '
            'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            (
                reference_id,
                gene.gene_name,

"""
Database persistence helpers for resistance rule imports — existence checks and feature lookup.
"""
...
def _rule_exists(
    ...
    *,
    ...
    row = conn.execute(
        ...
        'WHERE feature_id = ? AND drug_id = ? AND reference_identifier = ? '
        'AND position = ? AND reference = ? AND mutation = ? '
        'LIMIT 1',
        (feature_id, drug_id, reference_identifier, position, reference, mutation),
    ).fetchone()
...
def _build_feature_lookup(conn: sqlite3.Connection) -> dict[str, list[dict]]:
    """
    Build a feature lookup table from the project database.

    :param conn: SQLite database connection
    :return: dictionary mapping feature names to lists of feature rows
    """
    feature_lookup_rows = conn.execute(
        """
        SELECT
            g.id AS feature_id,
    ...
    for row in feature_lookup_rows:
        entries = [
            (row['feature_name'], 0),
            (row['locus_tag'], 1),
            (row['protein_id'], 2),
        ...
        for raw_key, alias_rank in entries:
            ...
            if not key or key in seen_keys:
                continue
            seen_keys.add(key)
            features_by_name.setdefault(key, []).append(
                {
